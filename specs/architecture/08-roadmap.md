...
| **4 — Workspace/RBAC Filtering** | `030-039` | Mandatory retrieval-time filter (`06-security-model.md` §4) — `workspace_id`-only after `012` — plus workspace-scoped Postgres RLS (deferred from `012`), RBAC route guards, authorization test suite | Phase 2 (needs `workspace_members`), Phase 3 (needs ingestion writing workspace-scoped payload) | Retrieval quality work in the next phase must be built and tested against an already-isolated system — building hybrid search first and bolting isolation on after risks shipping a retrieval change that accidentally bypasses filters |
...
| **10 — Restore Multi-Tenancy** | `090-099` | Re-introduce `tenants` / `tenant_members`, the tenant role ladder, `tenant_id` denormalization + Row-Level Security, and tenant-scoped `usage_quotas` / `api_keys` / `audit_log`; nest workspaces under tenants again | Phase 8 | `012` deferred multi-tenancy for assignment scope (see Phase 2). Restoring it is a schema + auth change best done once the product is otherwise service-shaped, authenticated, and observable — the same reasoning that put auth in Phase 2 rather than Phase 1 |
